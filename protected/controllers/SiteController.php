...
//                    Your doctor appointment is on $time dt. $DrName['first_name'].$DrName['last_name']. Pl ensure to be on time, else communicate any change well before. Thank you.
    ...
    public function actionGetPatientDetails() {
        ...
        if ($role_id == 3 || $role_id == 5) {
            ...
            if (empty($result)) {
                $queryresult = "";
                if ($is_clinic == "Y") {
                    $queryresult = Yii::app()->db->createCommand()->select('user_id,CONCAT( first_name," ",last_name) AS patient_name,mobile as patient_mobile,(select opd_consultation_fee from az_clinic_details where doctor_id=' . $userid . ' AND clinic_id = ' . $hospitalid . ') as apt_fees')->from('az_user_details')->where('role_id= 4 and mobile=:mobile', array(':mobile' => $mobilenumber))->queryAll();
                } else {
                    $queryresult = Yii::app()->db->createCommand()->select('user_id,CONCAT( first_name," ",last_name) AS patient_name,mobile as patient_mobile,(select doctor_fees from az_user_details where user_id=' . $userid . ' ) as apt_fees')->from('az_user_details')->where('role_id= 4 and mobile=:mobile', array(':mobile' => $mobilenumber))->queryAll();
                }
                $cmd['source'] = "exist";
    ...
    public function actionFindServicePatient() {
        $session = new CHttpSession;
        $session->open();
        $role_id = $session["user_role_id"];
        // $parent_hos_id=$session[""];
        ...
        $cmd = array();
        $result = Yii::app()->db->createCommand()
                ->select('book_id, patient_id, role_id, center_id, relation, other_relation_dis, full_name as patient_name, mobile_no as patient_mobile, patient_age, service_name, discription_doc, total_charges, collect_home, blood_group, no_of_unit, status, pincode, country_id, country_name, state_id, state_name, city_id, city_name, area_id,area_name, landmark, address, parent_hosp_id,DATE_FORMAT(appointment_date, "%d-%c-%Y") as preferred_day')->from('az_lab_book_details')->where('mobile_no=:mobile AND center_id=:center_id ', array(':mobile' => $mobilenumber, ':center_id' => $userid))->andWhere('status = :status ', array(':status' => 'Pending'))->queryAll();
        //  print_r($cmd);
        if (empty($result)) {
            $queryresult = "";
            $queryresult = Yii::app()->db->createCommand()->select('user_id,CONCAT( first_name," ",last_name) AS patient_name,mobile as patient_mobile,age')->from('az_user_details')->where('role_id= 4 and mobile=:mobile', array(':mobile' => $mobilenumber))->queryAll();
            $cmd['source'] = "exist";
            $cmd['resultset'] = $queryresult;
        } else {
            $cmd['source'] = "query";
