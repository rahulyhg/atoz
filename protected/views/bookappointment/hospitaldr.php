        ...
        <div class="row">
            ...
            <div class="container">
                ...
                <div class="form-group" style="margin-top:15px;"> <!--accept patient appointment details--> 
                    <label class="col-sm-2 control-label required" for="PatientAppointmentDetails_hospital_id">Doctors<span class="required">*</span></label>

                    <div class="col-sm-2">
                        <?php
                        $hospDrIdArr = array();
                        $doctHospital = Yii::app()->db->createCommand()->select("u.user_id,CONCAT(u.first_name,' ',u.last_name) doctorname")->from("az_user_details u")->where("parent_hosp_id = :id and role_id = 3 and is_active=1", array(":id" => $hid))->queryAll();
                        if (!empty($doctHospital)) {
                            foreach ($doctHospital as $key => $value) {
                                $hospDrIdArr[$value['user_id']] = $value['doctorname'];
                            }
                        }
                        echo $form->dropDownList($model,"doctor_id", $hospDrIdArr, array( "class"=> "form-control doctid"));
                        $model->hospital_id = $hid;
                        echo $form->hiddenField($model, 'hospital_id', array("class" => "hospitalselect hospitalid"));
                        $model->is_clinic = "N";
                ...
                <div class="form-group" style="margin-top:15px;">
                    ...
                    <label class ="col-sm-2 control-label">Doctor Fee<span class="required">*</span>:</label>
                    <div class="col-sm-2">
                        <?php
                    ...
                    if ($.isEmptyObject(resultdata)) {
                        ...
                        if(dataobj.source == "query") {
                            ...
                            $.each(dataobj.resultset, function (key, value) {
                                str += "<tr><td><input type='radio' name='patientlist' class='pradio'><input type='hidden' class='selectedsource' value='query'/><input type='hidden' class='querycreator' value='" + value.created_by + "'/></td><td class='pid hidden'> " + value.id + "</td><td class='fname'> " + value.patient_name + " </td><td class='mobile'> " + value.patient_mobile + "</td><td class='visittype'> " + value.type_of_visit + " </td><td class='docfees'>" + value.apt_fees + "</td><td class='aptdate'>" + value.preferred_day + "</td></tr>";
                            });
                            str += "</tbody></table>";
                            ...
                            if(dataobj.resultset == false) {
                                ...
                                $.each(dataobj.resultset, function (key, value) {
                                    str += "<tr><td><input type='radio' name='patientlist' class='pradio'><input type='hidden' class='selectedsource' value='exist'/><span class='docfees hidden'>" + value.apt_fees + "</span></td><td class='pid hidden'> " + value.user_id + "</td><td class='fname'> " + value.patient_name + " </td><td class='mobile'> " + value.patient_mobile + "</td></tr>";
                                });
                            }
